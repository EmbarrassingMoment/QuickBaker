...
QuickBakerのコードベースは単一責任原則（SRP）に従い、以下のコンポーネントで構成されています：

- **FQuickBakerModule**: モジュールのライフサイクル、メニュー登録、タブの生成を管理します。
- **FQuickBakerSettings**: ベイク設定（解像度、出力タイプなど）とバリデーションロジックを定義するデータ構造体です。
- **FQuickBakerCore**: レンダーターゲットのセットアップ、レンダリング、保存を含むコアとなるベイク処理のビジネスロジックを含みます。
- **FQuickBakerExporter**: 外部ファイル形式（PNG/EXR）へのテクスチャエクスポートを担当する専用クラスです。
- **SQuickBakerWidget**: ユーザーインタラクションと状態を管理するSlate UIウィジェットの実装です。
- **QuickBakerUtils**: 静的ユーティリティ関数（命名規則など）を提供します。
